...
This feature is supported for web endpoints with javadoc:org.springframework.boot.actuate.endpoint.web.annotation.WebPayload[format=annotation],
see the xref:actuator/endpoints.adoc#actuator.endpoints.implementing-custom.web.payload[complex types] section for more details.
...
[[actuator.endpoints.implementing-custom.web.payload]]
==== `@WebPayload` complex types as endpoint parameters

Web endpoints can accept complex types as method parameters. For that, custom types themselves must be annotated with
javadoc:org.springframework.boot.actuate.endpoint.web.annotation.WebPayload[format=annotation]:

include-code::../CustomPayload[]

Our `CustomPayload` can then be declared as a method parameter without any additional setup:

include-code::../MyWebEndpoint[tag=write]

NOTE: `@WebPayload` annotated types are not deserialized from the JSON request body directly,
so adding Jackson annotations on them will have no effect. Instead, data is first collected
from the request body or form parameters, and then bound to this custom type with
Spring Framework's javadoc:org.springframework.validation.DataBinder[].
