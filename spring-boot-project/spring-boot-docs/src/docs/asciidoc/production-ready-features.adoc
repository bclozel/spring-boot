...
Spring Boot Actuator will expose the "Liveness" and "Readiness" information with HTTP Probes using the `ProbesEndpoint`.
The Probes Actuator Endpoint is <<production-ready-endpoints-exposing-endpoints,enabled and exposed by default>>.

You can then point your `"livenessProbe"` to `"/actuator/probes/liveness"` and `"readinessProbe"` to `"/actuator/probes/readiness"`.
...
In case your application needs to rely on a specific subset of external checks for "Liveness", you should consider leveraging <<health-groups, Health Groups>> for that.
...
In this case, a Probe check could be successful even if the main application does not accept new connections.
