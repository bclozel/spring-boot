...
The internal state of Spring Boot applications is mostly reprensented by the Spring application context.
...
This typically happens during startup, while `CommandLineRunner` and `ApplicationRunner` are being processed, or at any time if the application decides that it's too busy for additional traffic.
...
----
    @Component
	public class LocalCacheVerifier implements ApplicationEventPublisherAware {
...
Spring Boot provides <<production-ready-features.adoc#production-ready-kubernetes-probes,Kubernetes HTTP probes for "Liveness" and "Readiness" with Actuator Endpoints>>.
