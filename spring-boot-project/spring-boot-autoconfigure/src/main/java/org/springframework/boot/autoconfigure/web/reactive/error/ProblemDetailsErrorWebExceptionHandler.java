...
/**
 * Global {@link org.springframework.web.server.WebExceptionHandler} that renders
 * {@link ErrorAttributes} as {@link ProblemDetail} responses.
 * <p>
 ...
 * <p>
 * This implementation will render errors as HTML views if the client explicitly supports
 ...
 * <p>
 * If the client doesn't support HTML, the error information will be rendered as a
 * {@code "application/problem+json"} payload.
 *
 * @author Brian Clozel
 * @since 3.5.0
 */
public class ProblemDetailsErrorWebExceptionHandler extends AbstractErrorWebExceptionHandler {
	...
	/**
	 * Create a new {@code ProblemDetailsErrorWebExceptionHandler} instance.
	 * @param errorAttributes the error attributes
	 * @param resources the resources configuration properties
	 * @param errorProperties the error configuration properties
	 * @param applicationContext the current application context
	 */
	public ProblemDetailsErrorWebExceptionHandler(ErrorAttributes errorAttributes, Resources resources,
	...
	protected Mono<ServerResponse> renderErrorView(ServerRequest request) {
		ProblemDetail problemDetail = getProblemDetail(request, MediaType.TEXT_HTML);
		...
		return Flux.fromIterable(getViewNames(status))
			.flatMap((viewName) -> renderErrorView(viewName, responseBody, problemDetail))
			.switchIfEmpty(this.errorProperties.getWhitelabel().isEnabled()
					? renderDefaultErrorView(responseBody, problemDetail) : Mono.error(getError(request)))
			.next();
	}

	private List<String> getViewNames(int errorStatus) {
	...
	/**
	 * Render the error information as a Problem Detail payload.
	 * @param request the current request
	 * @return a {@code Publisher} of the HTTP response
	 */
	protected Mono<ServerResponse> renderErrorResponse(ServerRequest request) {
		ProblemDetail problemDetail = getProblemDetail(request, MediaType.ALL);
		int status = problemDetail.getStatus();
	...
	private ProblemDetail getProblemDetail(ServerRequest request, MediaType mediaType) {
		return getProblemDetail(request, getErrorAttributeOptions(request, mediaType));
	}
	...
	/**
	 * Predicate that checks whether the current request explicitly supports
