...
/**
 * Tests for {@link BasicErrorHandlerFunctions} using {@link MockMvcTester} and
 * {@link SpringBootTest @SpringBootTest}.
 *
 * @author Brian Clozel
 */
@SpringBootTest(properties = { "server.error.include-message=always", "spring.mvc.problemdetails.enabled=true" })
@DirtiesContext
class BasicErrorHandlerFunctionsMockMvcTests {
	...
	void testDirectAccessForMachineClient() {
		assertThat(this.mvc.get().uri("/error")).hasStatus5xxServerError().bodyText().contains("500");
	}
