...
class ErrorMvcAutoConfigurationTests {
	...
	@Nested
	class BasicErrorControllerTests {
		...
		void renderContainsViewWithExceptionDetails() {
			this.contextRunner.run((context) -> {
				...
				errorView.render(errorAttributes.getErrorAttributes(webRequest, withAllOptions()),
	...
	@Nested
	class ErrorRouterFunctionTests {
		...
		@Test
		void contributesRouterFunction() {
			this.contextRunner.run((context) -> assertThat(context).hasBean("errorRouterFunction"));
		}
		...
		void renderContainsViewWithExceptionDetails() {
			this.contextRunner.run((context) -> {
				...
				DispatcherServletWebRequest webRequest = createWebRequest(
						new IllegalStateException("Exception message"), false);
				ServerRequest serverRequest = ServerRequest.create(webRequest.getRequest(), Collections.emptyList());
				errorView.render(Map.of("problem", errorAttributes.asProblemDetail(serverRequest, withAllOptions())),
						serverRequest.servletRequest(), webRequest.getResponse());
				assertThat(webRequest.getResponse().getContentType()).isEqualTo("text/html;charset=UTF-8");
				String responseString = ((MockHttpServletResponse) webRequest.getResponse()).getContentAsString();
				assertThat(responseString).contains(
						"<p>This application has no explicit mapping for /error, so you are seeing this as a fallback.</p>")
		...
		void renderCanUseJavaTimeTypeAsTimestamp() { // gh-23256
			this.contextRunner.run((context) -> {
				...
				DispatcherServletWebRequest webRequest = createWebRequest(
						new IllegalStateException("Exception message"), false);
				ServerRequest serverRequest = ServerRequest.create(webRequest.getRequest(), Collections.emptyList());
				ProblemDetail problemDetail = errorAttributes.asProblemDetail(serverRequest, withAllOptions());
				problemDetail.setProperty("timestamp", Clock.systemUTC().instant());
				errorView.render(Map.of("problem", problemDetail), webRequest.getRequest(), webRequest.getResponse());
				assertThat(webRequest.getResponse().getContentType()).isEqualTo("text/html;charset=UTF-8");
				String responseString = ((MockHttpServletResponse) webRequest.getResponse()).getContentAsString();
				assertThat(responseString).contains("This application has no explicit mapping for /error");
			});
