...
/**
 * Tests for {@link BasicErrorHandlerFunctions} using a real HTTP server.
 *
 * @author Brian Clozel
 * @author Phillip Webb
 * @author Dave Syer
 ...
 */
class BasicErrorHandlerFunctionsIntegrationTests {
	...
	@Test
	@Disabled("exception handled by MVC ProblemDetails")
	@SuppressWarnings({ "rawtypes", "unchecked" })
	...
	private void assertErrorAttributes(Map<?, ?> content, String status, String error, Class<?> exception,
		...
		assertThat(content.get("title")).as("Wrong error").isEqualTo(error);
		if (exception != null) {
		...
		assertThat(content.get("detail")).as("Wrong message").isEqualTo(message);
		assertThat(content.get("instance")).as("Wrong path").isEqualTo(path);
	}
