	...
	public void autoConfiguredDataSourceIsInstrumented() {
		this.contextRunner
				...
				.run((context) -> {
					...
					assertThat(registry.mustFind("data.source.max.connections")
							.tags("name", "dataSource").meter()).isNotNull();
				});
	}
	...
	public void autoConfiguredDataSourceWithCustomMetricName() {
		this.contextRunner
				...
				.run((context) -> {
					context.getBean(DataSource.class).getConnection().getMetaData();
					MeterRegistry registry = context.getBean(MeterRegistry.class);
					assertThat(registry.mustFind("custom.name.max.connections")
							.tags("name", "dataSource").meter()).isNotNull();
				});
	}
	...
	public void allDataSourcesCanBeInstrumented() {
		this.contextRunner.withUserConfiguration(TwoDataSourcesConfiguration.class)
				...
				.run((context) -> {
					...
					MeterRegistry registry = context.getBean(MeterRegistry.class);
					assertThat(registry.mustFind("data.source.max.connections")
							.tags("name", "first").meter()).isNotNull();
					assertThat(registry.mustFind("data.source.max.connections")
							.tags("name", "secondOne").meter()).isNotNull();
				});
	}
